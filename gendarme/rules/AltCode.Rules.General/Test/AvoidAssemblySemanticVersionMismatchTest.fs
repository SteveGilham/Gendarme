...
type AvoidAssemblySemanticVersionMismatchTest() =
  ...
  member val private assembly : AssemblyDefinition = null
    with get, set

  [<OneTimeSetUp>]
  member this.FixtureSetUp () =
    let unit = System.Reflection.Assembly.GetExecutingAssembly().Location
  ...
  member this.EmptyCustomAttributes () =
    let cac = this.assembly.CustomAttributes
              |> Seq.toList
    try
  ...
  member this.AbsentAssemblyFileVersion () =
    let afv =
      ...
      |> Seq.find(fun ca -> ca.AttributeType.FullName = "System.Reflection.AssemblyFileVersionAttribute")
    try
      Assert.That(this.assembly.CustomAttributes.Remove afv, "AssemblyFileVersionAttribute not found")
      base.AssertRuleFailure (this.assembly, 1)
      Assert.AreEqual (Severity.Medium, this.Runner.Defects.[0].Severity, "Medium")
    finally
...
// fsharplint:disable  NonPublicValuesNames
  ...
  member this.VersionMismatch () =
    ...
    let s = DefinitionLoader.GetTypeDefinition (typeof<string>)
    ...
    try
     ...
     let file8_2_18_22015 = CustomAttributeArgument(s, "8.2.18.22015")
     ...
     let file8_2_1_22015 = CustomAttributeArgument(s, "8.2.1.22015")
